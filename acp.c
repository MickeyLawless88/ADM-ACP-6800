...
#define NAMELEN   16
#define MAXSYM    2000
#define MAXREF    12000          /* total symbol references tracked for XREF */
#define OBJMAX    64             /* max object bytes a single line can emit */

/* Listing geometry.  LST_LINEMAX is only the DEFAULT width; the effective
 * width is the runtime `list_width`, adjustable from the ACP shell.  A width
 * of 0 means "unlimited" - no source clipping and no XREF reference wrap.
 * The fixed columns are ADDR(4) + 2 + CODE(12) + 2 + LINE(4) + 2 = 26.     */
#define LST_LINEMAX  79
#define LST_PREFIX   26
#define LST_SRCMAX   (LST_LINEMAX - LST_PREFIX)   /* default source columns  */

static int list_width = LST_LINEMAX;   /* effective width; 0 => unlimited    */

/* One format string drives every data row so the columns can never drift.
 * Field widths: ADDR 4, CODE 12 (fixed by the .12 precision so over-long
 * code can't push later columns), LINE 4.  Header/rule below must match.  */
#define LST_ROW_FMT  "%04X  %-12.12s  %4d  %s\n"

/* Header and rule share the same field widths as the data row (4/12/4 with
 * two-space gaps) so the labels sit exactly over their columns.           */
#define LST_HDR_FMT  "%-4s  %-12s  %-4s  %s\n"
...
/* ---- symbol table ---------------------------------------------------- */
#define ST_LAB 0                 /* address label (code or initialised data)*/
#define ST_EQU 1                 /* EQU constant (absolute value)           */
#define ST_MEM 2                 /* RMB reserved storage (a variable)       */
typedef struct { char name[NAMELEN]; long val; int defined; int defline;
                 int type; } SYM;
static SYM *symtab;              /* malloc'd: MAXSYM entries               */
static int nsym;

/* ---- cross-reference store ------------------------------------------- *
 * One (symbol,line) pair per operand use.  Filled only during pass 2 so a
 * symbol is not counted twice (pass 1 also evaluates expressions).  The
 * pairs are sorted and de-duplicated when the XREF table is printed.       */
typedef struct { int sym; int line; } XREF;
static XREF *xref;               /* malloc'd: MAXREF entries               */
static int  nxref;
static int  xref_on;             /* 1 => eval() records references          */
static int  xref_curline;        /* display line eval() is working on        */
static int  g_defline;           /* display line sym_define() is defining on  */
static int  g_symtype;           /* kind (ST_*) of the symbol being defined   */
...
static int sym_define(const char *name, long val)
{
    ...
    symtab[nsym].defline = g_defline;
    symtab[nsym].type    = g_symtype;
    nsym++;
    return 0;
}
...
{
    if (t == ST_EQU) return "EQU";
    if (t == ST_MEM) return "MEM";
    return "LAB";
}

/* record one operand reference to sym index while assembling xref_curline */
static void xref_note(int symidx)
{
    if (!xref_on || nxref >= MAXREF) return;
    xref[nxref].sym  = symidx;
    xref[nxref].line = xref_curline;
    nxref++;
}
...
static void pass1(void)
{
    ...
    nxref = 0; xref_on = 0;             /* references are collected in pass 2 */
    ...
    for (i = 2; i <= last_dispno(); i++) {
        ...
        g_defline = i;                 /* line any label here is defined on   */
        g_symtype = ST_LAB;            /* default; EQU and RMB override below  */
        ...
        if (lab[0]) {
            if (dir == DIR_RMB) g_symtype = ST_MEM;   /* reserved storage    */
            if (sym_define(lab, (long)loc) == -1)
                emit_err(NULL, i, "LABEL ALREADY EXISTS", lab);
        }
...
/* Copy src into dst (buffer >= LINELEN+2), clipped so the whole listing line
 * stays within the effective list_width.  A shortened line ends in '>' to
 * flag the truncation.  When list_width is 0 (unlimited) the source is
 * copied in full and the row may run past 80 columns by design.            */
static const char *fit_src(char *dst, const char *src)
{
    int budget = (list_width > 0) ? list_width - LST_PREFIX : -1;
    int n = (int)strlen(src);
    if (budget < 0 || n <= budget) {        /* unlimited, or it already fits */
        strcpy(dst, src);
    } else if (budget <= 0) {               /* no room for any source at all */
        dst[0] = '\0';
    } else {
        memcpy(dst, src, (size_t)(budget - 1));
        dst[budget - 1] = '>';
        dst[budget]     = '\0';
    }
    return dst;
...
static void pass2(FILE *lst)
{
    ...
    xref_on = 1;                 /* eval() now records symbol references     */
    ...
    for (i = 0; i < nprog; i++) {
        ...
        char lsrc[LINELEN + 2];      /* clipped (or full) src for the listing */
        const char *opnd;

        xref_curline = a->dispno;    /* references on this line credit here  */
        parse_line(src, lab, mn, opbuf);   /* parses the full, unclipped src */
        fit_src(lsrc, src);                /* clipped copy for the listing   */
        opnd = opbuf;
        nb = 0;
        ...
        if (a->is_dir == DIR_EQU || a->is_dir == DIR_ORG ||
            ...
            printf(LST_ROW_FMT, here, code, a->dispno, lsrc);
            if (lst) fprintf(lst, LST_ROW_FMT, here, code, a->dispno, lsrc);
...
/* write s to the terminal and, when open, to the listing file */
static void xout(FILE *lst, const char *s)
{
    fputs(s, stdout);
    if (lst) fputs(s, lst);
}

/* Symbol table, up to five entries across.  Each cell is NAME(8) + value and
 * occupies a 15-column stride (with the 2-space indent), so the number that
 * fits is list_width/15, capped at the requested 5 (5 => 75 cols).          */
static void print_labels(FILE *lst)
{
    int  i, col = 0;
    int  percol = (list_width > 0) ? list_width / 15 : 5;
    char line[128], cell[32];

    if (percol > 5) percol = 5;
    if (percol < 1) percol = 1;

    xout(lst, "\nSYMBOL TABLE\n");
    strcpy(line, "  ");                      /* leading indent               */
    for (i = 0; i < nsym; i++) {
        sprintf(cell, "%-8.8s %04lX", symtab[i].name, symtab[i].val & 0xFFFF);
        strcat(line, cell);
        if (++col == percol) {               /* row full -> flush            */
            strcat(line, "\n"); xout(lst, line);
            strcpy(line, "  "); col = 0;
        } else {
            strcat(line, "  ");              /* gap before next cell         */
        }
    }
    if (col > 0) { strcat(line, "\n"); xout(lst, line); }
}

/* ---- cross-reference table ------------------------------------------- *
 * Alphabetical list of every symbol with its kind, value, defining line,
 * reference count, and every line that references it.  Reference lists wrap
 * under the REFERENCES column to honour the effective list_width (0 = no
 * wrap, subject only to the output-buffer cap).                            *
 * Columns: SYMBOL 14, TYPE 3, VAL 4, DEF 5, CNT 4, then refs; 2-space gaps. */
#define XREF_HDR_FMT  "%-14s  %-3s  %-4s  %-5s  %4s  %s\n"
#define XREF_PREFIX   40           /* chars before the REFERENCES field     */
#define XREF_TOKW     6            /* width of one "Lnnnn " reference token  */
#define XREF_LINEBUF  512          /* build buffer for one physical row     */

/* references per physical line for the current width (>= 1, buffer-capped) */
static int xref_perline(void)
{
    int cap = (XREF_LINEBUF - XREF_PREFIX - 2) / XREF_TOKW;   /* buffer limit */
    int n;
    if (list_width <= 0) return cap;                          /* unlimited    */
    n = (list_width - XREF_PREFIX) / XREF_TOKW;
    if (n < 1)   n = 1;
    if (n > cap) n = cap;
    return n;
}
...
{
    const XREF *x = (const XREF *)a, *y = (const XREF *)b;
    if (x->sym != y->sym) return x->sym - y->sym;
    return x->line - y->line;
}

static int symname_cmp(const void *a, const void *b)
{
    return strcmp(symtab[*(const int *)a].name, symtab[*(const int *)b].name);
}

static int xref_lower(int symidx)   /* first xref[] slot with sym >= symidx  */
{
    int lo = 0, hi = nxref;
    while (lo < hi) {
        int mid = (lo + hi) / 2;
        if (xref[mid].sym < symidx) lo = mid + 1; else hi = mid;
    }
    return lo;
}

static void print_xref(FILE *lst)
{
    int  *order;
    int   k, j;
    int   perline = xref_perline();
    char  line[XREF_LINEBUF], tok[16], defstr[16], refrule[64];

    if (nsym == 0) return;

    /* group references by symbol, ascending line within each symbol */
    if (nxref > 1) qsort(xref, (size_t)nxref, sizeof(XREF), xref_cmp);

    /* print symbols alphabetically without disturbing the symbol table */
    order = (int *)malloc((unsigned)nsym * sizeof(int));
    if (!order) return;
    for (k = 0; k < nsym; k++) order[k] = k;
    qsort(order, (size_t)nsym, sizeof(int), symname_cmp);

    xout(lst, "\nCROSS-REFERENCE TABLE\n");
    sprintf(line, XREF_HDR_FMT, "SYMBOL", "TYP", "VAL", "DEF", "CNT",
            "REFERENCES");
    xout(lst, line);
    {
        int nd = perline * XREF_TOKW;        /* underline the usable ref area */
        if (nd < 4)  nd = 4;
        if (nd > 48) nd = 48;
        memset(refrule, '-', (size_t)nd); refrule[nd] = '\0';
    }
    sprintf(line, XREF_HDR_FMT, "--------------", "---", "----", "-----",
            "----", refrule);
    xout(lst, line);

    for (k = 0; k < nsym; k++) {
        SYM *s = &symtab[order[k]];
        int  count = 0, emitted = 0, prev = -1;
        int  refs = 0;

        if (s->defline > 0) sprintf(defstr, "L%d", s->defline);
        else                strcpy(defstr, "-");

        /* first walk: how many distinct lines reference this symbol */
        for (j = xref_lower(order[k]); j < nxref && xref[j].sym == order[k]; j++) {
            if (xref[j].line == prev) continue;
            prev = xref[j].line; refs++;
        }
        prev = -1;

        /* first physical line carries SYMBOL / TYP / VAL / DEF / CNT */
        sprintf(line, "%-14.14s  %-3s  %04lX  %-5s  %4d  ",
                s->name, sym_type_name(s->type), s->val & 0xFFFF, defstr, refs);

        /* second walk: the reference tokens, wrapped under REFERENCES */
        for (j = xref_lower(order[k]); j < nxref && xref[j].sym == order[k]; j++) {
            if (xref[j].line == prev) continue;       /* one hit per line     */
            prev = xref[j].line;
            if (count == 0 && emitted) {              /* continuation indent  */
                memset(line, ' ', XREF_PREFIX);
                line[XREF_PREFIX] = '\0';
            }
            sprintf(tok, "L%d", xref[j].line);
            sprintf(line + strlen(line), "%-*s", XREF_TOKW, tok);
            if (++count == perline) {
                strcat(line, "\n"); xout(lst, line);
                emitted = 1; count = 0;
            }
        }
        if (count > 0) { strcat(line, "\n"); xout(lst, line); emitted = 1; }
        if (!emitted)  { strcat(line, "\n"); xout(lst, line); }
    }
...
static void do_assemble(void)
{
    ...
    if (lst) {
        ...
        fprintf(lst, LST_HDR_FMT, "ADDR", "CODE", "LINE", "SOURCE STATEMENT");
        fprintf(lst, LST_HDR_FMT, "----", "------------", "----", "----------------");
    }
    printf(LST_HDR_FMT, "ADDR", "CODE", "LINE", "SOURCE STATEMENT");
    printf(LST_HDR_FMT, "----", "------------", "----", "----------------");
...
/* ACP option 4: set the maximum listing width (0 = unlimited) */
static void set_list_width(void)
{
    char ans[LINELEN];
    int  w, floor = XREF_PREFIX + 10;   /* room for the XREF header label     */

    if (list_width > 0) printf("\nLISTING WIDTH IS %d COLUMNS.\n", list_width);
    else                printf("\nLISTING WIDTH IS UNLIMITED.\n");
    printf("(72=TTY  80=CRT  132=PRINTER  0=UNLIMITED, RETURN=KEEP)\n");
    if (!getline_p("...WIDTH ? ", ans, sizeof ans)) return;
    rstrip(ans);
    if (ans[0] == '\0') { printf("WIDTH UNCHANGED.\n"); return; }

    w = atoi(ans);
    if (w < 0) w = 0;
    if (w > 0 && w < floor) {
        printf("TOO NARROW - MINIMUM IS %d COLUMNS (OR 0 FOR UNLIMITED).\n",
               floor);
        return;
    }
    list_width = w;
    if (list_width > 0) printf("LISTING WIDTH SET TO %d COLUMNS.\n", list_width);
    else                printf("LISTING WIDTH SET TO UNLIMITED.\n");
}
...
{
   char w[24];
   if (list_width > 0) sprintf(w, "%d", list_width); else strcpy(w, "OFF");
   ...
   printf("4    SET LISTING WIDTH   (NOW %s)\n", w);
   printf("5    END\n");
}
...
int main(void)
{
    ...
    xref     = (XREF *)malloc((unsigned)MAXREF   * sizeof(XREF));
    if (!gl_lines || !prog || !symtab || !xref) {
...

{
    ...
    for (;;) {
        ...
        case '4': set_list_width(); break;
        case '5': printf("##\n"); printf("SIGNED OFF.\n"); return 0;
        default:  printf("INVALID OPTION - ENTER 1, 2, 3, 4 OR 5\n"); break;
        }
    }
    return 0;
}
